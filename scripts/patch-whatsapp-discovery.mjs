...
if (start < 0 || end < 0) throw new Error('WhatsApp discovery function boundaries not found')
...
const replacement = String.raw`async function discoverWhatsAppData(
  ...
  try {
    ...
    const token = debugData?.data
    const granularScopes = token?.granular_scopes
    ...
    console.info('WhatsApp Meta token inspected:', { is_valid: token?.is_valid, app_id: token?.app_id, has_whatsapp_management: hasManagement, has_whatsapp_messaging: hasMessaging, whatsapp_target_count: targetIds.length })
  } catch (error) {
