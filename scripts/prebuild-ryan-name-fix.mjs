...
const helper = `function explicitNameAnswer(history: RyanTurn[], currentMessage: string) {\n  const text = clean(currentMessage, 120)\n  const lastModel = history.filter((item) => item.role === 'model').at(-1)?.text || ''\n  if (!text || !/(اسمك|اسم حضرتك|اسمك بالكامل|الاسم)/i.test(lastModel)) return ''\n  if (text.length > 40 || /[0-9@+]/.test(text) || /[.!؟?،,]/.test(text)) return ''\n  const words = text.split(/\\s+/).filter(Boolean)\n  if (words.length < 1 || words.length > 3) return ''\n  return reliableName(text)\n}\nfunction obviousAdIntent(value: string) {\n  return /(?:عايز|عاوز|محتاج|اعمل|نعمل|ابدأ|ابدا|إعلان|اعلان|إعلانات|اعلانات|حملة إعلانية|حمله اعلانيه|تسويق)/i.test(value)\n}\nfunction genericContinuation(value: string) {\n  return /(?:نكمل من آخر نقطة|نكمل من اخر نقطه|نبدأ من جديد|أقدر أساعدك في إيه|اقدر اساعدك في ايه|قولي حابب نساعدك|تمام، نكمل)/i.test(value)\n}\nfunction repairContextReply(value: string, currentMessage: string, state: { lastQuestion: string; topic: string }, customerName: string, explicitNameCaptured: boolean, leadCreated: boolean) {\n  if (leadCreated) return value\n  if (explicitNameCaptured && /(?:اسمك|اسم حضرتك|اسمك بالكامل|الاسم)/i.test(value)) return `تشرفت يا ${customerName}، تحب نساعدك في أنهي خدمة؟`\n  if (obviousAdIntent(currentMessage) && (genericContinuation(value) || /(?:اسم حضرتك|اسمك|الاسم)/i.test(value))) return 'تمام يا فندم، الإعلان هيكون لمنتج أو خدمة إيه؟'\n  if (genericContinuation(value) && state.topic && !/(?:تحب|ممكن|إيه|ايه|إيه نوع|ايه نوع)/i.test(value)) return `تمام يا فندم، تحب نكمل في موضوع ${state.topic} ولا محتاج حاجة تانية؟`\n  return value\n}\n`
...
} else if (!source.includes('function repairContextReply(')) {
  throw new Error('Ryan source contains partial context patch')
}
