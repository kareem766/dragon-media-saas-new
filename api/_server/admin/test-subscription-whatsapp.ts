...
export default async function handler(req: any, res: any) {
  ...
  const templatesResponse = await fetch(`https://graph.facebook.com/${graphVersion}/${encodeURIComponent(wabaId)}/message_templates?name=${encodeURIComponent(templateName)}&limit=100`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const template = candidates.find((item: any) => String(item?.name || '') === templateName && String(item?.status || '').toUpperCase() === 'APPROVED' && String(item?.language || '') === language) || candidates.find((item: any) => String(item?.name || '') === templateName && String(item?.status || '').toUpperCase() === 'APPROVED')
  ...
  buttons.forEach((button: any, buttonIndex: number) => {
    ...
    const count = distinctVariableCount(templateUrl)
    if (count <= 0) return
    ...
    components.push({ type: 'button', sub_type: 'url', index: String(buttonIndex), parameters: values.slice(0, count).map((text) => ({ type: 'text', text: cleanText(text) })) })
  })
  const response = await fetch(`https://graph.facebook.com/${graphVersion}/${encodeURIComponent(phoneNumberId)}/messages`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ messaging_product: 'whatsapp', recipient_type: 'individual', to: phone, type: 'template', template: { name: templateName, language: { code: String(template.language || language) }, components } }) })
  const data = await response.json().catch(() => ({}))
  if (!response.ok) {
    const metaError = data?.error ? { code: data.error.code, type: data.error.type, subcode: data.error.error_subcode, message: data.error.message, errorUserTitle: data.error.error_user_title, errorUserMsg: data.error.error_user_msg, fbtraceId: data.error.fbtrace_id, details: data.error?.error_data?.details } : undefined
    console.error('[subscription-whatsapp-test] Meta send rejected', { status: response.status, templateName, templateLanguage: template.language, templateStatus: template.status, templateCategory: template.category, components, metaError })
    return json(res, 502, { success: false, error: data?.error?.message || `Meta send failed (${response.status})`, metaError, templateName, templateLanguage: template.language })
  }
  return json(res, 200, { success: true, test: true, organizationId, phoneMasked: phone.length > 4 ? `${phone.slice(0, 3)}****${phone.slice(-2)}` : '****', externalId: String(data?.messages?.[0]?.id || ''), templateName })
}
