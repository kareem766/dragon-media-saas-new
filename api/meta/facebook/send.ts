...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: secretRow, error: secretError } = await db.from('system_secrets').select('value')
      .eq('key', 'whatsapp_outbound_webhook_secret').maybeSingle()
    ...
    if (messageError) { console.error('Facebook outbound message lookup failed', messageError); return errorResponse(res, 500, 'Unable to load message') }
    ...
    if (message.sender_type !== 'agent' && !isRyanMessage) return res.status(200).json({ ok: true, skipped: true, reason: 'not_outbound_agent_or_ryan_message' })
    if (message.external_id) return res.status(200).json({ ok: true, skipped: true, reason: 'already_sent', external_id: message.external_id })

    const { data: conversation, error: conversationError } = await db.from('conversations')
      .select('id, organization_id, channel, customer_id').eq('id', message.conversation_id).maybeSingle()
    if (conversationError || !conversation) return errorResponse(res, 404, 'Conversation not found')
    if (conversation.channel !== 'facebook' && conversation.channel !== 'messenger') return res.status(200).json({ ok: true, skipped: true, reason: 'not_facebook_conversation' })

    const { data: customer, error: customerError } = await db.from('customers')
      .select('id, phone, name').eq('id', conversation.customer_id).eq('organization_id', conversation.organization_id).maybeSingle()
    ...
    const { data: connection, error: connectionError } = await db.from('meta_connections')
      .select('id, access_token, status, metadata').eq('organization_id', conversation.organization_id)
      .eq('provider', 'facebook').eq('status', 'connected').order('updated_at', { ascending: false }).limit(1).maybeSingle()
    if (connectionError || !connection?.access_token) return errorResponse(res, 422, 'Facebook connection is not ready', 'FACEBOOK_NOT_READY')

    const connectionMetadata = connection.metadata && typeof connection.metadata === 'object' ? (connection.metadata as Record<string, unknown>) : {}
    ...
    if (!metaResponse.ok) {
      ...
      console.error('Facebook Messenger send failed', { status: metaResponse.status, error: payload?.error ?? payload, messageId, organizationId: conversation.organization_id })
      await db.from('messages').update({ metadata: { ...metadata, facebook_outbound: true, outbound_status: 'failed', outbound_error: errorMessage } }).eq('id', message.id)
    ...
    await db.from('messages').update({ external_id: externalId, metadata: { ...metadata, facebook_outbound: true, outbound_status: 'accepted', facebook_page_id: pageId, facebook_recipient_id: recipientId, meta_message_id: externalId } }).eq('id', message.id)
    await db.from('conversations').update({ last_message_at: now, updated_at: now }).eq('id', conversation.id).eq('organization_id', conversation.organization_id)
