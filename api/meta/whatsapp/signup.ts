...
<body>
  ...
  <script>
    ...
    // CRITICAL: Meta requires the redirect_uri to be byte-for-byte identical
    // between the authorization dialog and the token exchange. Never include
    // the dynamic OAuth state query parameter in redirect_uri.
    ...
    async function submitWhenReady() {
      ...
      try {
        const response = await fetch('/api/meta/whatsapp/complete', {
          ...
          cache: 'no-store',
          body: JSON.stringify({state, code: authorizationCode, waba_id: sessionInfo?.waba_id || null, phone_number_id: sessionInfo?.phone_number_id || null, business_id: sessionInfo?.business_id || null}),
        });
        const contentType = response.headers.get('content-type') || '';
        const data = contentType.includes('application/json') ? await response.json().catch(() => null) : {error: await response.text().catch(() => '')};
    ...
    function launchSignup() {
      ...
      if (!state) { setStatus('تعذر التحقق من جلسة الربط. أعد المحاولة من الإعدادات.', 'error'); return; }
      button.disabled = true;
    ...
    (function(d,s,id){const firstScript=d.getElementsByTagName(s)[0];if(d.getElementById(id))return;const script=d.createElement(s);script.id=id;script.async=true;script.defer=true;script.crossOrigin='anonymous';script.src='https://connect.facebook.net/en_US/sdk.js';if(firstScript&&firstScript.parentNode)firstScript.parentNode.insertBefore(script,firstScript);else d.head.appendChild(script)})(document,'script','facebook-jssdk');
  </script>
</body>
</html>`
