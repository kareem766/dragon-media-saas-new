...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!organizationId) {
      ...
      if (membershipError) return json(res, 500, { error: 'تعذر التحقق من الشركة المرتبطة بالحساب.' })
    ...
    // Embedded Signup is launched through the Meta JavaScript SDK. The SDK uses
    // this configuration ID and returns the short-lived authorization code plus
    // WABA/phone IDs through the WA_EMBEDDED_SIGNUP session event.
