...
function normalizePhone(v:unknown){return String(v||'').replace(/[\\u200e\\u200f\\u202a-\\u202e\\s+]/g,'').replace(/[^0-9]/g,'')}
...
export default async function handler(req:any,res:any){
  ...
  const response=await fetch(`https://graph.facebook.com/${graphVersion}/${encodeURIComponent(phoneNumberId)}/messages`,{method:'POST',headers:{Authorization:`Bearer ${decryptToken(integration.config.access_token)}`,'Content-Type':'application/json'},body:JSON.stringify({messaging_product:'whatsapp',recipient_type:'individual',to:phone,type:'template',template:{name:templateName,language:{code:language},components:[{type:'body',parameters:[{type:'text',text:String(org.name||'عميلنا')},{type:'text',text:'اختبار التجديد'},{type:'text',text:'يوم واحد'},{type:'text',text:new Date().toISOString().slice(0,10)}]},{type:'button',sub_type:'url',index:'0',parameters:[{type:'text',text:'test'}]}]}})})
  ...
  return json(res,200,{success:true,test:true,organizationId,phoneMasked:phone.length>4?`${phone.slice(0,3)}****${phone.slice(-2)}`:'****',externalId,templateName})
}
