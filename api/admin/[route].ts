...
async function ryanInbox(req: VercelRequest, res: VercelResponse) {
  ...
  if (!aiBody && process.env.GROQ_API_KEY) {
    ...
    try {
      ...
      aiBody = { candidates: [{ content: { parts: [...(choice?.message?.content ? [{ text: choice.message.content }] : []), ...((choice?.message?.tool_calls || []).map((call: any) => ({ functionCall: { name: call?.function?.name, args: (() => { try { return JSON.parse(call?.function?.arguments || '{}') } catch { return {} } })() } })))] } }], usageMetadata: { totalTokenCount: Number(data?.usage?.total_tokens || 0) || 0 } }
    } catch (error) { console.error('Ryan inbox Groq request failed', error); return res.status(503).json({ error: 'محرك Ryan غير متاح مؤقتًا.' }) } finally { clearTimeout(timeout) }
  }
  ...
  let actionTaken: string | null = null; let reply = parts.map((part: any) => String(part?.text || '')).join('').trim()

  for (const call of calls) {
    try {
      const args = asMap(call.args); let result: any
      if (call.name === 'update_customer_memory') {
        if (rememberCustomer && customer?.id) {
          const patch: JsonMap = {}
          for (const key of ['name', 'company', 'activity', 'service_interest', 'goal', 'budget', 'location', 'preferences', 'pain_points', 'important_notes', 'last_intent', 'summary']) { const value = clean(args[key], 1500); if (value) patch[key] = value }
          if (Object.keys(patch).length) result = await supabase.from('customers').update({ ai_memory: { ...memory, ...patch, updated_at: new Date().toISOString(), source: 'ryan' }, updated_at: new Date().toISOString() }).eq('id', customer.id).eq('organization_id', organizationId)
        }
      } else if (call.name === 'create_lead') result = await supabase.rpc('ai_create_lead', { p_organization_id: organizationId, p_conversation_id: conversationId, p_name: clean(args.name) || customerName, p_phone: clean(args.phone) || customerPhone, p_service: clean(args.service) || clean(memory.service_interest), p_activity: clean(args.activity) || clean(memory.activity), p_goal: clean(args.goal) || clean(memory.goal), p_notes: clean(args.notes) || clean(memory.summary), p_created_by: ownerUserId || null })
      ...
      else if (call.name === 'book_appointment') { const serviceName = clean(args.service_name); const date = clean(args.date); const time = clean(args.time); if (!serviceName || !date || !time) continue; result = await supabase.rpc('ai_book_appointment', { p_organization_id: organizationId, p_conversation_id: conversationId, p_service_name: serviceName, p_date: date, p_time: time, p_created_by: ownerUserId || null }) }
      else result = await supabase.rpc('ai_request_handoff', { p_organization_id: organizationId, p_conversation_id: conversationId, p_customer_name: customerName, p_reason: clean(args.reason) || 'Customer requested human assistance', p_created_by: ownerUserId || null })
      if (!result?.error) { if (call.name !== 'update_customer_memory') actionTaken = call.name; if (call.name === 'book_appointment') reply = 'تمام، سجلت لك الموعد بنجاح.'; else if (call.name === 'request_human_handoff') reply = 'أكيد، هحوّل المحادثة لحد من الفريق ويتابع معاك.'; else if (call.name === 'create_deal') reply = 'تمام، سجلت طلبك وهنتابع معاك بخصوص التعاقد.'; else if (call.name === 'create_lead') reply = 'تمام، سجلت بياناتك وهنتابع معاك بخصوص طلبك.' }
    } catch (error) { console.error('Ryan inbox tool error', error) }
  }
  ...
  void supabase.rpc('record_ryan_usage', { p_organization_id: organizationId, p_conversation_id: conversationId, p_user_id: ownerUserId || null, p_model: model, p_event_type: 'message', p_tokens: Number(aiBody?.usageMetadata?.totalTokenCount || 0) || 0, p_cost: 0, p_metadata: { source: 'whatsapp_inbox', provider, action: actionTaken, inbound_message_id: messageId } }).then(({ error }) => { if (error) console.error('Ryan usage record failed', error) })
