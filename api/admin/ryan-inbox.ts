...
const nameStop=/^(?:تمام|حاضر|ماشي|اه|أه|ايوه|أيوه|السلام عليكم|السلام عليكم ورحمة الله وبركاته|وعليكم السلام|اهلا|أهلا|أهلًا|منور|ممكن|عايز|عاوز|محتاج|الخدمة|خدمة|السعر|سعر|بكام|بكم|كام|شكرا|شكراً|شكرا جدا|شكراً جداً|شكراً ليك|شكرا ليك|متشكر|متشكر جداً|العفو|تسلم|تسلم جدا|ربنا يخليك|ربنا يكرمك|يومك سعيد|تمام شكرا|تمام شكراً)$/iu
...
${knowledgeText||'لا توجد معلومات في قاعدة المعرفة حالياً.'}`
 ...
 const lastModelMessage=(history.slice().reverse().find((t:any)=>t.role==='model')?.parts?.[0]?.text||'').toString();const nameQuestionPending=/(?:اسم حضرتك|اسمك|اسمِك|الاسم|أسم حضرتك|أسمك)/iu.test(lastModelMessage);const hasKnownNameBeforeTurn=Boolean((isWhatsApp?rememberedExplicitName:storedCustomerName)&&looksLikeName(isWhatsApp?rememberedExplicitName:storedCustomerName)&&!invalidCustomerName(isWhatsApp?rememberedExplicitName:storedCustomerName));
 // Never treat an arbitrary short customer message as a name. A bare name is accepted only as the direct answer to Ryan's name question; explicit forms such as "اسمي كريم" are handled by extractName(). This prevents phrases like "شكرا جدا" from corrupting the CRM name.
