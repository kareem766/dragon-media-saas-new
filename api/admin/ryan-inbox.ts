...
${knowledgeText||'لا توجد معلومات في قاعدة المعرفة حالياً.'}`
 ...
 let reply=text(plan.reply,5000);if(hasTrustedName){const escapedName=effectiveName.replace(/[.*+?^${}()|[\]\\]/g,'\\let reply=text(plan.reply,5000);if(!actionResult.success)');const shortNamePattern=new RegExp('(يا\\s+)?(أستاذ|استاذ|أستاذة|استاذة)\\s+[^\\s،,!?؟]{1,'+Math.max(1,effectiveName.length-1)+'}(?=[،,!?؟\\s]|$)','gu');if(shortNamePattern.test(reply)&&!reply.includes(effectiveName)){reply=reply.replace(shortNamePattern,(m,prefix,title)=>`${prefix||''}${title} ${effectiveName}`)}}if(!actionResult.success){console.error('Ryan action failed',text(plan.action,60),actionResult.message||'unknown');reply='حصلت مشكلة بسيطة وأنا بنفذ الطلب، ومش هقول لحضرتك إنه تم قبل ما يتنفذ فعلاً.'}
