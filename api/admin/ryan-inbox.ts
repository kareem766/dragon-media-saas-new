...
${knowledgeText||'لا توجد معلومات في قاعدة المعرفة حالياً.'}`
 ...
 const durableMemory={name:(isWhatsApp?(rememberedExplicitName||explicitName):text(customer.name,120))||null,name_source:(isWhatsApp?(rememberedExplicitName||explicitName)?'customer_explicit':null):(explicitName?'customer_explicit':'crm'),phone:cleanPhone(text(customer.phone,80))||null,service:text(plan.service,160)||null,budget:text(plan.budget,120)||budgetFromText(current)||null,intent:text(plan.intent,100)||null,last_message:current,updated_at:new Date().toISOString()};if(rememberCustomer)await supabase.from('ai_agent_memory').upsert({agent_id:agent.id,customer_id:customer.id,memory:durableMemory,summary:`${durableMemory.name||'العميل'} — ${durableMemory.service||'الخدمة غير محددة'}${durableMemory.budget?' — '+durableMemory.budget:''}`},{onConflict:'agent_id,customer_id'})
