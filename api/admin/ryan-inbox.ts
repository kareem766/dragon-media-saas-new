...
export default async function main(req:VercelRequest,res:VercelResponse){
 ...
 const {data:ownerRows}=await supabase.from('users').select('email,is_platform_admin,active').eq('organization_id',organizationId).eq('is_platform_admin',true).eq('active',true).limit(10)
 const ownerRow=(ownerRows||[]).find((row:any)=>String(row?.email||'').trim().toLowerCase()==='kalnoby0@gmail.com')
 const isPlatformOwner=Boolean(ownerRow?.is_platform_admin===true && ownerRow?.active!==false)
 let quotaRow:any
 if(isPlatformOwner){
