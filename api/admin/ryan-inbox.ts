...
PERSONA: ${persona}`
   ...
   if(effectiveNeedsHuman){
    ...
    const {data:notifyUsers}=await supabase.from('users').select('id').eq('organization_id',organizationId).eq('active',true)
    if(notifyUsers?.length){
      await supabase.from('notifications').insert(notifyUsers.map((u:any)=>({organization_id:organizationId,user_id:u.id,type:'ryan_handoff',title:'طلب تدخل بشري من Ryan',body:`العميل ${text(customer.name,120)||'عميل جديد'} طلب التواصل مع موظف. ${handoffReason||''}`.trim(),message:`تم تحويل محادثة Ryan إلى موظف بشري. العميل: ${text(customer.name,120)||'عميل جديد'}`,link:`/inbox?conversation=${conversationId}`,entity_type:'conversation',entity_id:conversationId,is_read:false})))
    }
