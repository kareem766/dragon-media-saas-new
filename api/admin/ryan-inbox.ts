...
async function analyzeConversation(geminiKey:string,model:string,system:string,history:Turn[],current:string){
 ...
 try{
  ...
  for(const candidate of candidates){
   try{
    const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(candidate)}:generateContent?key=${encodeURIComponent(geminiKey)}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:system}]},contents:[...history,{role:'user',parts:[{text:current}]}],generationConfig:{maxOutputTokens:700,responseMimeType:'application/json',responseSchema:{type:'object',properties:{lead_intent:{type:'boolean'},intent:{type:'string'},needs_human:{type:'boolean'},handoff_reason:{type:'string'},is_advertising:{type:'boolean'},name:{type:'string'},phone:{type:'string'},email:{type:'string'},company:{type:'string'},service:{type:'string'},budget:{type:'string'},goal:{type:'string'},business_activity:{type:'string'},follow_up_at:{type:'string'},appointment_date:{type:'string'},appointment_time:{type:'string'},automation_name:{type:'string'},automation_hours:{type:'string'},automation_priority:{type:'string'},missing:{type:'array',items:{type:'string'}},complete:{type:'boolean'},next_action:{type:'string'},action:{type:'string',enum:['continue','update_customer','follow_up','schedule_appointment','create_automation']},reply:{type:'string'},confidence:{type:'number'}},required:['lead_intent','intent','needs_human','handoff_reason','is_advertising','name','phone','email','company','service','budget','goal','business_activity','follow_up_at','appointment_date','appointment_time','automation_name','automation_hours','automation_priority','missing','complete','next_action','action','reply','confidence'],additionalProperties:false}}})})
    const d=await r.json().catch(()=>({}))
    if(r.ok){
