...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Never delete an account that has already been linked to a workspace.
    ...
    if (linkedUserError) {
      return json(res, 500, { error: 'تعذر التحقق من حالة الحساب.' })
    }

    if (linkedUser) {
      return json(res, 409, { error: 'الحساب مرتبط بمساحة عمل بالفعل.' })
    }

    const { error: deleteError } = await admin.auth.admin.deleteUser(userId, false)
    if (deleteError) {
      return json(res, 500, { error: 'تعذر إلغاء الحساب غير المكتمل.' })
    }
