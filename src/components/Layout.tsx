...
function SubscriptionExpiredScreen({ daysRemaining, formattedRenewalDate }: { daysRemaining: number | null; formattedRenewalDate: string | null }) {
  return <div dir="rtl" className="min-h-[calc(100vh-140px)] flex items-center justify-center py-8 sm:py-10 dm-page-enter"><div className="w-full max-w-xl"><div className="bg-white border border-sand-200 rounded-3xl shadow-sm p-6 sm:p-10 text-center"><div className="mx-auto mb-6 w-16 h-16 rounded-2xl bg-red-50 flex items-center justify-center text-red-600 text-2xl">!</div><h2 className="text-2xl sm:text-3xl font-bold text-ink-950 mb-3">انتهى اشتراكك</h2><p className="text-sand-600 leading-7 mb-6">انتهت مدة باقتك الحالية وتم إيقاف الوصول إلى مميزات Dragon Media. يمكنك تجديد اشتراكك الآن لاستعادة الوصول.</p>{formattedRenewalDate && <div className="bg-sand-50 rounded-2xl px-4 py-3 mb-6 text-sm text-sand-700">تاريخ انتهاء الاشتراك: <span className="font-semibold text-ink-950">{formattedRenewalDate}</span></div>}<div className="flex flex-col sm:flex-row gap-3 justify-center"><Link to="/plans" className="inline-flex items-center justify-center rounded-xl bg-ink-950 px-6 py-3 text-sm font-semibold text-white hover:opacity-90">تجديد الاشتراك</Link><Link to="/billing" className="inline-flex items-center justify-center rounded-xl border border-sand-300 bg-white px-6 py-3 text-sm font-semibold text-ink-950 hover:bg-sand-50">الفواتير والمدفوعات</Link></div>{daysRemaining !== null && <p className="mt-4 text-xs text-sand-500">انتهاء الاشتراك: {daysRemaining === 0 ? 'اليوم' : `${daysRemaining} يوم`}</p>}</div></div></div>
}
...
export default function Layout() {
  ...
  return <div dir="rtl" className="flex h-screen min-h-0 overflow-hidden bg-sand-50"><Sidebar open={open} onClose={() => setOpen(false)} /><div className="flex-1 flex min-w-0 min-h-0 flex-col"><Topbar title={title} onMenuClick={() => setOpen(true)} />{!loading && isActive && <SubscriptionBanner daysRemaining={daysRemaining} formattedRenewalDate={formattedRenewalDate} />}{showPendingBanner && <div role="status" aria-live="polite" className="bg-gold-500/15 border-b border-gold-500/30 px-3 sm:px-4 md:px-8 py-3"><div className="mx-auto max-w-[1600px] flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-ink-950">طلب الاشتراك قيد المراجعة</p><p className="text-xs text-sand-600 mt-0.5">سيتم تفعيل مميزات الباقة بعد تأكيد الدفع.</p></div><Link to="/billing" className="shrink-0 rounded-xl px-4 py-2 bg-gold-100 text-sm font-bold text-gold-700 hover:bg-gold-200">متابعة الدفع</Link></div></div>}{!loading && isExpired && !exempt && !adminRoute && <div role="alert" className="bg-red-50 border-b border-red-200 px-3 sm:px-4 md:px-8 py-3"><div className="mx-auto max-w-[1600px] flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-red-800">انتهى اشتراكك وتم إيقاف مميزات المنصة</p><p className="text-xs text-red-700/80 mt-0.5">جدد اشتراكك لاستعادة الوصول.</p></div><Link to="/plans" className="shrink-0 rounded-xl px-4 py-2 bg-red-100 text-sm font-bold text-red-700 hover:bg-red-200">تجديد الاشتراك</Link></div></div>}<main className="flex-1 min-h-0 overflow-y-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6"><div className="mx-auto w-full max-w-[1600px] min-h-full dm-page-enter" key={pathname}>{shouldLockPage ? <SubscriptionExpiredScreen daysRemaining={daysRemaining} formattedRenewalDate={formattedRenewalDate} /> : <Outlet />}</div></main></div></div>
}
