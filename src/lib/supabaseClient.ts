...
const paymentRequestLabels: Record<
  string,
...
export default function Billing() {
  ...
  /*
   * supabaseClient is narrowed once here so TypeScript knows
   * that all Supabase calls below are made only when the client exists.
   */
  ...
  const loadBillingData = useCallback(async () => {
    ...
    try {
      const [requestsResult, invoicesResult] =
        await Promise.all([
          supabaseClient
      ...
      if (loadedInvoices.length > 0) {
        const invoiceIds = loadedInvoices.map(
          (invoice) => invoice.id
        )
      ...
      setError(
        'تعذر تحميل بيانات الفوترة حاليًا. حاول مرة أخرى.'
      )
    } finally {
      setDataLoading(false)
    }
  ...
  const loading =
    subscriptionLoading || dataLoading
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-5">
          ...
          <div className="text-2xl font-bold text-ink-950 mt-2">
            {paymentRequests.length.toLocaleString(
              'ar-EG'
            )}
          </div>
