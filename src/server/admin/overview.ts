...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    /*
     * organizations is the core query.
     * If it fails, the dashboard cannot show
     * company data, so return a clear error.
     */
    if (orgsRes.error) {
      console.error(
      ...
      return res.status(500).json({
        ...
        details: orgsRes.error.message,
      })
    }

    /*
     * The remaining tables should not make the
     * whole dashboard unusable if one optional
