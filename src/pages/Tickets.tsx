...
export default function Tickets() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (messageError) {
      setSaving(false)
      setError(
        'تم إنشاء التذكرة، لكن تعذر إضافة الرسالة. يمكنك فتح التذكرة وإرسال الرد مرة أخرى.'
      )
      setForm({
        subject: '',
        priority: 'متوسطة',
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="h-7 w-32 rounded-lg bg-sand-100 animate-pulse" />
            <div className="h-4 w-56 rounded-lg bg-sand-100 animate-pulse" />
          </div>

          <div className="h-10 w-32 rounded-xl bg-sand-100 animate-pulse" />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[1, 2, 3].map(item => (
            <div
              key={item}
              className="h-24 rounded-2xl border border-sand-200 bg-white animate-pulse"
            />
          ))}
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div className="min-w-0">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-ink-900 text-sand-50 flex items-center justify-center shrink-0">
                    <svg
                      width="19"
                      height="19"
                      viewBox="0 0 24 24"
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-ink-950">
            الدعم الفني
          </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <Card className="p-4">
          <div className="text-xs text-ink-900/45">المفتوحة</div>
          <div className="text-2xl font-bold text-ink-950 mt-1">
            {openCount}
          </div>
        </Card>
        ...
        <Card className="p-4 col-span-2 sm:col-span-1">
          <div className="text-xs text-ink-900/45">المغلقة / المحلولة</div>
          <div className="text-2xl font-bold text-ink-950 mt-1">
            {resolvedCount}
          </div>
        </Card>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                      ...
                      <div className="shrink-0">
                        <Badge
                          tone={statusTones[ticket.status] || 'default'}
                        >
                          {statusLabels[ticket.status] || ticket.status}
                        </Badge>
                      </div>
