...
() => tasks.filter(task => task.status === ‘مكتملة’).length,
[tasks]
)
  ...
  const { data, error: insertError } = await supabase
    ...
    .insert({
      ...
      reminder_at: taskForm.reminder_at
        ? new Date(taskForm.reminder_at).toISOString()
        : null,
  ...
  <Card className="p-5 lg:p-6">
    ...
    {tasks.length === 0 ? (
      ...
      <div className="space-y-3">
        {tasks.map(task => {
          const assignedTaskUser = users.find(
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <div className="flex flex-wrap gap-4 mt-3 text-xs text-ink-900/45">
                    <span>
                      الموعد: {formatTaskDueDate(task.due_date)}
                    </span>
                    {assignedTaskUser && (
                      <span>
                        المسؤول:{' '}
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  ...
                  {isCompleted && (
                    <button
                      onClick={() =>
                        updateTaskStatus(task, 'قيد التنفيذ')
                      }
                      disabled={taskSaving}
        ...
        <div className="p-6 border-b border-sand-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-ink-900/45 mt-1">
                المهمة سيتم ربطها تلقائيًا بالعميل {customer.name}
              </p>
            </div>
            <button
